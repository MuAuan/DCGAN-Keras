...
def main(BATCH_SIZE=55, ite=1000):
    ...
    opt = optimizer=Adam(lr=0.0001, beta_1=0.5) #"SGD" # (lr=0.01, decay=1e-4, momentum=0.2, nesterov=True)
    ...
    for i in range(0 * 1000,31 * 1000):
        ...
        noise = np.random.uniform(size=[batch_size, 10*10], low=-1.0, high=1.0) #32*32
...
def generate(BATCH_SIZE=55, ite=10000, nice=False):
    ...
    if nice:
        ...
        noise = np.random.uniform(-1, 1, (BATCH_SIZE*20, 10*10)) ##32*32
